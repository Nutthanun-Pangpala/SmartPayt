    ...
    useEffect(() => {
        const fetchUserDetails = async () => {
            try {
                ...
                const response = await axios.get(`http://localhost:3000/admin/users/${lineUserId}`, {
                    headers: {
        ...
        const fetchUserAddresses = async () => {
            try {
                ...
                const response = await axios.get(`http://localhost:3000/admin/users/address/${lineUserId}`,{
                    headers: {
                        'Cache-Control': 'no-cache',
                        'authorization': `Bearer ${token}`,
                    }
                ...
                const billsData = await Promise.all(
                    addresses.map(async (address) => {
                        try {
                            const res = await axios.get(`http://localhost:3000/admin/users/address/bills/${address.address_id}`,{
                                headers: {
                                    'Cache-Control': 'no-cache',
                                    'authorization': `Bearer ${token}`,
                                }
                            });
                            const unpaidBills = res.data.bills.filter((bill) => bill.status !== "ยังชำระแล้ว"); // กรองบิลที่ยังไม่ชำระ
    ...
    const handleVerifyAddress = async (addressId) => {
        try {
            ...
            const response = await axios.patch(
                `http://localhost:3000/admin/users/address/verify/${addressId}`,{
                    headers: {
                        'Cache-Control': 'no-cache',
                        'authorization': `Bearer ${token}`,
                    }
            ...
            if (response.data.success) {
                // เมื่อยืนยันสำเร็จให้รีเฟรชที่อยู่และบิล
                ...
                setError('เกิดข้อผิดพลาดในการยืนยันที่อยู่');
            }
        } catch (error) {
    ...
    return (
        <div className="flex flex-col min-h-screen bg-gray-100">
            <div className="flex items-center justify-between p-4 bg-white shadow">
                <button onClick={() => navigate('/admin')} className="text-gray-800 p-2">
                    กลับไปยังหน้าหลัก
                </button>
                                ...
                                <div onClick={() => toggleExpand(address.address_id)} className="cursor-pointer">
                                    ...
                                    <div className="flex">
                                        <p className={address.address_verified ? "text-green-500 " : "text-red-500"}>
                                            <i className="fi fi-sr-shield-trust"></i> สถานะการยืนยันที่อยู่: {address.address_verified ? "ยืนยันแล้ว" : "ยังไม่ยืนยัน"}
                                        </p>
                                        {!address.address_verified && (
                                        <button
                                            onClick={() => handleVerifyAddress(address.address_id)}
                                            ...
                                            <table className="table-auto w-full border-collapse">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th className="px-4 py-2 border-b">วันที่ครบกำหนด</th>
                                                        <th className="px-4 py-2 border-b">สถานะ</th>
                                                    </tr>
                                                </thead>
                                                <tbody >
                                                    {billsMap[address.address_id] && billsMap[address.address_id].map((bill, index) => (
                                                    <tr key={index}>
                                                        <td className="px-4 py-2 border-b">{bill.amount_due} บาท</td>
                                                        <td className="px-4 py-2 border-b">
                                                        {bill.due_date ? new Date(bill.due_date).toLocaleDateString() : "ไม่ระบุ"}
                                                        </td>
                                                        <td className={bill.status ? "px-4 py-2 border-b text-green-500" : "px-4 py-2 border-b text-red-500"}>
